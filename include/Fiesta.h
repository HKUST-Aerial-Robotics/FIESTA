...
template<class DepthMsgType>
Fiesta<DepthMsgType>::Fiesta(ros::NodeHandle node) {
...
#endif
  // For Jie Bao
  transform_sub_ = node.subscribe("/vins_estimator/camera_pose", 10, &Fiesta::TransformCallback, this);
  depth_sub_ = node.subscribe("/camera/depth/image_rect_raw", 10, &Fiesta::DepthCallback, this);
//    transform_sub_ = node.subscribe("pointcloud", 10, &Fiesta::TransformCallback, this);
//    depth_sub_ = node.sub scribe("depth", 10, &Fiesta::DepthCallback, this);

  // Cow_and_Lady
  // depth_sub_ = node.subscribe("/camera/depth_registered/points", 1000, PointcloudCallback);
  // transform_sub_ = node.subscribe("/kinect/vrpn_client/estimated_transform", 1000, TransformCallback);

  //EuRoC
//    depth_sub_ = node.subscribe("/dense_stereo/pointcloud", 1000, PointcloudCallback);
//    transform_sub_ = node.subscribe("/vicon/firefly_sbx/firefly_sbx", 1000, TransformCallback);
...
template<class DepthMsgType>
void Fiesta<DepthMsgType>::Visualization(ESDFMap *esdf_map, bool global_vis, const std::string &text) {
  if (esdf_map != nullptr) {
...
template<class DepthMsgType>
void Fiesta<DepthMsgType>::RaycastProcess(int i, int part, int tt) {
  Eigen::Vector3d half = Eigen::Vector3d(0.5, 0.5, 0.5);
  for (int idx = part * i; idx < part * (i + 1); idx++) {
    std::vector<Eigen::Vector3d> output;
    if (idx > cloud_.points.size()) break;
...
template<class DepthMsgType>
void Fiesta<DepthMsgType>::RaycastMultithread() {
...
template<class DepthMsgType>
double Fiesta<DepthMsgType>::GetInterpolation(const cv::Mat &img, double u, double v) {
...
template<class DepthMsgType>
void Fiesta<DepthMsgType>::DepthConversion() {
...
template<class DepthMsgType>
void Fiesta<DepthMsgType>::SynchronizationAndProcess() {
  ros::Time depth_time;
  ...
  while (!depth_queue_.empty()) {
    ...
    if (transform_queue_.empty() || std::get<0>(transform_queue_.front()) <= depth_time + ros::Duration(time_delay)) {
      break;
    }
    if (!new_pos) {
      depth_queue_.pop();
...
template<class DepthMsgType>
void Fiesta<DepthMsgType>::TransformCallback(const geometry_msgs::PoseStamped::ConstPtr &msg) {
  Eigen::Vector3d pos;
  Eigen::Quaterniond q;
...
template<class DepthMsgType>
void Fiesta<DepthMsgType>::DepthCallback(const DepthMsgType &depth_map) {
  depth_queue_.push(depth_map);
  SynchronizationAndProcess();
}

template<class DepthMsgType>
void Fiesta<DepthMsgType>::UpdateEsdfEvent(const ros::TimerEvent & /*event*/) {
  if (!new_msg_) return;
  new_msg_ = false;
  cur_pos_ = sync_pos_;
